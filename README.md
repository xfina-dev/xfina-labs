...
| NSE (nseindia.com), ETF price history | 2026-09-26 | Prohibits "systematic or automated data collection activities (including scraping, data mining, data extraction and data harvesting)", with no personal-use exception, and bars use of the data for "gaming, virtual trading or simulation" (aimed at gaming and virtual-trading products; backtesting is not clearly covered). Built anyway on the maintainer's decision that a browser macro doing the same clicks, one file at a time, in the user's own session, is not a service or scraper. The card says plainly what the terms say and leaves the choice to the reader. Re-check if NSE objects or the terms change. |
...
**A bookmark is made once and reused.** It has no dates or settings baked in (NSE Indices covers equity indexes from Total returns Index Values and debt indexes from Historical Index Data, whose files are the index level rather than a total-return series): it covers the supported indexes (or, for NSE, ETF symbols) selected in the download list (drag it again if the selection changes; its memory carries over). On the site it opens a small panel with three modes. *Update* (the default) fetches only what is new: it remembers, per index, the newest date that came back (in that site's browser storage) and starts the day after it, running to today, and an index it has never done gets its full history, so the first click brings in everything and later clicks bring in the difference. *Full history* redoes everything. *Custom* takes a start and end date. The panel shows the files it will download before Start. Any run, custom ones included, moves the memory forward to the newest date it saved, so after a custom 1 to 10 Sep the next Update starts on 11 Sep. The importer lets newer files replace older data for the same dates, so any overlap or repeat is harmless. The memory is per browser and per site address: clearing that site's data makes the next Update a full history. NSE serves both `niftyindices.com` and `www.niftyindices.com`, and browser storage is separate for each, so the bookmark always works on the `www` address (clicked on the other one it moves the page there and asks for another click). If the browser blocks storage the panel says so, and Update fetches the full history each time. The link is long (about 18 KB encoded), which the mainstream browsers accept.

**Pacing.** Like a person, the bookmark works the page's own form in view (each menu item and field is scrolled to and outlined in green as it is used). For each file it fills the form, waits, presses Submit, lets the results show, waits again, then saves the file. The two waits are halves of a random 6 to 9 seconds, and a countdown ring, always visible, shows them. Without folder access, after the third file the run also waits about 15 seconds (or until Continue now is pressed) so the browser's "allow multiple downloads" prompt can be answered. It has to run in the foreground tab: browsers pause background tabs, and NSE Indices' page does not load results in a hidden one, so parallel tabs do not work. In Chrome or Edge the panel asks for one folder and writes every file there; elsewhere the browser saves them by its own setting.
