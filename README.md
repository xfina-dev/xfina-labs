...
**Pacing.** Like a person, the bookmark works the page's own form in view (each menu item and field is scrolled to and outlined in green as it is used), waits 6 to 9 seconds (a countdown ring that is always visible), presses Submit, saves the file as soon as the page has rendered, and moves straight on to filling the next form. Without folder access, after the third file the run also waits about 15 seconds (or until Continue now is pressed) so the browser's "allow multiple downloads" prompt can be answered. It has to run in the foreground tab: browsers pause background tabs, and NSE Indices' page does not load results in a hidden one, so parallel tabs do not work. In Chrome or Edge the panel asks for one folder and writes every file there; elsewhere the browser saves them by its own setting.
