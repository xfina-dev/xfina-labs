...
The bookmark is generated from the datasets the user added to their download list and the card's settings (`__NAME__` tokens in the script are replaced with JSON when the link is built). Currently: `nse-indices.js` (Nifty 50, Next 50, Midcap 150, Smallcap 250 Total Returns Index). NSE Indices' page refuses a range longer than a year (more than 365 days between the two dates), so it downloads one CSV per financial year (April to March), from where the user chooses to start. A whole financial year always fits, leap years included. The browser decides where the files are saved.
