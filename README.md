...
The bookmark is generated from the datasets the user added to their download list, so it fetches exactly those (`__NAME__` tokens in the script are replaced with JSON when the link is built). Currently: `nse-indices.js` (Nifty 50, Next 50, Midcap 150, Smallcap 250 Total Returns Index, one JSON file each, the full history in one request).
