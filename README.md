...
The bookmarklet paces itself gently (a few seconds between files), except that the first three files go out back to back so the browser's "allow multiple downloads" prompt appears early; it then waits about 15 seconds (or until the user presses Continue now) before carrying on at the gentler pace. Native date inputs need `color-scheme: dark` in dark mode (set in `labs.css`) or their calendar icon disappears. One bookmark per site works through every dataset the user added, one after another, with one progress bar. It has to run in the foreground tab: browsers pause background tabs, and NSE Indices' page does not load results in a hidden one, so parallel tabs do not work. The period is chosen in the card (current financial year by default, previous, full history, or a custom start and end date from the browser's date pickers); the current and previous financial year are worked out when the bookmark is clicked, so a repeat run covers whatever is current then; the importer lets newer files replace older data for the same dates.
...
The bookmark is generated from the datasets the user added to their download list and the card's settings (`__NAME__` tokens in the script are replaced with JSON when the link is built). Currently: `nse-indices.js` (Nifty 50, Next 50, Midcap 150, Smallcap 250 Total Returns Index). NSE Indices' page refuses a range longer than a year (more than 365 days between the two dates), so it downloads one CSV per financial year (April to March) within the chosen period. A whole financial year always fits, leap years included. The browser decides where the files are saved.
