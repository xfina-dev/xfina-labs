...
// the URL hash, e.g. #equity/us/etf/irish, so a link lands on the same step.
const cls = ref(null);
const region = ref(null);
const vehicle = ref(null);
const listing = ref(null);

const needsListing = computed(() => vehicle.value === 'etf' && cls.value && region.value && hasListings(cls.value, region.value));
const ready = computed(() => cls.value && region.value && vehicle.value && (!needsListing.value || listing.value));
const result = computed(() => (ready.value ? datasetsFor(cls.value, region.value, vehicle.value, listing.value) : { items: [], note: '' }));
const count = (v) => (cls.value && region.value ? datasetsFor(cls.value, region.value, v, v === 'etf' ? 'us' : undefined).items.length + (v === 'etf' && hasListings(cls.value, region.value) ? datasetsFor(cls.value, region.value, 'etf', 'irish').items.length : 0) : null);

// Steps after a change are cleared, because their options depend on the earlier answers.
const pick = (which, v) => {
  ...
  if (which === 'vehicle') { vehicle.value = v; listing.value = null; }
...
const fromHash = () => {
  ...
  vehicle.value = region.value && VEHICLES.some((x) => x.id === v) ? v : null;
  listing.value = vehicle.value === 'etf' && LISTINGS.some((x) => x.id === l) ? l : null;
};
watch([cls, region, vehicle, listing], () => { try { history.replaceState(null, '', `#${toHash()}`); } catch { /* ignore */ } });

// What the user added, by dataset id. Kept in this browser only.
const STORE = 'xfina_labs_guide_selection_v2';
const picked = ref([]);
...
// Datasets in a step, grouped by their heading (Nifty 50, Short duration, ...).
const groups = computed(() => {
  const m = new Map();
  for (const i of result.value.items) m.set(i.group, [...(m.get(i.group) || []), i]);
  return [...m.entries()];
});
            ...
            <div class="grid gap-6 md:grid-cols-2">
              <div v-for="[g, items] in groups" :key="g" class="space-y-2">
                ...
                <ul class="divide-y rounded-md border">
                  <li v-for="i in items" :key="i.id" class="flex flex-wrap items-center justify-between gap-3 p-3">
                    <div class="min-w-0">
                      <div class="font-medium">{{ i.name }} <Tag v-if="i.code">{{ i.code }}</Tag></div>
                      <div class="text-xs text-muted-foreground mt-0.5">{{ i.ccy }} · {{ i.ret }}</div>
                    </div>
